...
class TurboFeed
{
	...
	public function getItemsFilter()
    {
		...
		/*bind events*/
		foreach(GetModuleEvents("yandex.turboapi", "OnFeedOneStepElementFilterBefore", true) as $arEvent)
		{
				...
				*/
				...
				if($this->isXmlProfile)
				{
					if($this->feed['TYPE'] === 'ya_zen')
					{
						...
						{
							$propName = 'enclosure';
							$propAttr = \Yandex\Export\ProfileTools::getDetailMedia($arItem['DETAIL_TEXT'], $arField, $arItem, $this->feed);
						}
					}
				...
				else
				{
					...
					{
						$propUnit = Loc::getMessage('YANDEX_TYRBO_API_FIELDS_NOT_UNIT_VALUE');
					}
				}
				...
				*/
				...
				if($arField['USE_BOOLEAN'] == 'Y') 
				{
					if(!is_array($value)) 
					{
						...
						$value = ($value == 'Y' || $value == 1 || $value > 0 || $value == 'true' || ToLower($value) == Loc::getMessage('YANDEX_TYRBO_API_TEMP_YES_VALUE') ? $valueTrue : $valueFalse);
    ...
    {	
		...
		if(!$fp = @fopen($fileName, "wb"))
		{
			...
			{
				$directory = \Bitrix\Main\IO\Directory::createDirectory(\Yandex\TurboAPI\Turbo::getPath().'/'.$arFields['ID'].'/');
				...
				{
					$runError = str_replace('#FILE#', $fileName, Loc::getMessage('YANDEX_TYRBO_API_FILE_OPEN_WRITING'));
				}
			}
		}
	...
	protected function getDirTemplates($templates = '.default')
	{
		...
		$arTemplateList = \Yandex\TurboAPI\Turbo::getTemplateList($type);
		...
		{
			$pathToAction = $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/yandex.turboapi/load/templates/'.$type.'/'.$templates;
		}
		...
		if($directory->isExists())
		{
			...
			foreach($arChildren as $k => $file)
			{
				if($file->isExists())
				{
					...
					$result = \Yandex\TurboAPI\Model\Request::addFeed($arFields['LID'], 'production', $data, $this->isGzip, $this->feed['FIELDS']['HOST_ID_SUBDOMAIN']);
					if(is_array($result) && isset($result['task_id']))
					{
						$resultTask = \Yandex\TurboAPI\TaskTable::add(array('FEED_ID' => $arFields['ID'], 'LID' => $arFields['LID'], 'TASK_ID' => $result['task_id'], 'MODE' => 'PRODUCTION'));
						if($resultTask->isSuccess())
						{
							...
							if(rename($file->getPhysicalPath(), $this->getPath().'/reports/'.$newName))
							{
								...
								\Yandex\TurboAPI\TaskTable::update($taskId, array('NAME' => $newName));
							}
							unset($newName);
						}
						$j++;
			...
			\Yandex\TurboAPI\FeedTable::update($arFields['ID'], array('IS_NOT_UPLOAD_FEED' => ($count - $j > 0 ? 'Y' : 'N')));
		}
		return array(
			'TOTAL_FILE' => $count,
			'PROCESSED' => $i,
    ...
    {	
        ...
        $str .= '<rss xmlns:yandex="http://news.yandex.ru" xmlns:media="http://search.yahoo.com/mrss/" xmlns:turbo="http://turbo.yandex.ru" version="2.0">';
        $str .= "<channel>\n";
		$path = \Yandex\TurboAPI\Turbo::getPath().'/'.$fileName;
		if(!$fp = @fopen($path, "wb"))
		{
			$directory = \Bitrix\Main\IO\Directory::createDirectory(\Yandex\TurboAPI\Turbo::getPath().'/');
			if($directory->isExists())
			{
				$fp = @fopen($path, "wb");
			}
			...
			{
				$runError = str_replace('#FILE#', $path, Loc::getMessage('YANDEX_TYRBO_API_FILE_OPEN_WRITING'));
			}
		}
		if($fp)
		{
			...
			if($file->isExists())
			{
				...
				$result = \Yandex\TurboAPI\Model\Request::addFeed($lid, 'production', $data, $this->isGzip);
				if(is_array($result) && isset($result['task_id']))
				{
					$resultTask = \Yandex\TurboAPI\TaskTable::add(array('FEED_ID' => 0, 'LID' => $lid, 'TASK_ID' => $result['task_id'], 'MODE' => 'PRODUCTION'));
					if($resultTask->isSuccess())
					{
						$taskId = $resultTask->getId();
						$newName = str_replace('.xml', '_'.$taskId.'.xml', $file->getName());
