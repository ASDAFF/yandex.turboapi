...
class TaskTable extends Entity\DataManager
{
	...
	{
		return 'yandex_yandex_turbo_task';
	}
	...
	public static function getMap()
	{
		return array(
			...
			'FEED_ID' => array(
				'data_type' => 'integer',
				'title' => Loc::getMessage('YANDEX_TYRBO_API_ENTITY_FEED_ID_FIELD'),
			),
			'LID' => array(
				'data_type' => 'string',
			...
			'TASK_ID' => array(
				'data_type' => 'string',
				'title' => Loc::getMessage('YANDEX_TYRBO_API_ENTITY_TASK_ID_FIELD'),
			),
			'MODE' => array(
				'data_type' => 'string',
				'title' => Loc::getMessage('YANDEX_TYRBO_API_ENTITY_MODE_FIELD'),
				'hidden' => true,
			),
	...
	public static function OnDelete(\Bitrix\Main\Entity\Event $event)
    {
        ...
        if(intval($task['ID']) > 0)
		{
			$arTask = \Yandex\TurboAPI\TaskTable::getById($task['ID'])->fetch();
			\Bitrix\Main\IO\File::deleteFile(\Yandex\TurboAPI\Turbo::getPath().'/reports/'.$arTask['NAME']);
		}
    }
}