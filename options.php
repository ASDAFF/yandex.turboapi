			...
			<td colspan="2">
				...
				foreach($siteList as $val)
				{
					...
					<table cellspacing="5" cellpadding="0" border="0" width="100%" align="center">
						<tr>
							<th align="right" width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_TOKEN")?>:</th>
						...
						<?if(strlen($arTurboProp['token']) > 0):?>
							<tr>
								<th align="right" width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_USER")?></th>
								<td width="60%">
									<?if(strlen($arTurboProp['user_id']) > 0):?>
										<?=$arTurboProp['user_id']?>
										<input value="<?=$arTurboProp['user_id']?>" name="turbo[<?=$val["ID"]?>][host_id]" type="hidden">
									<?else:?>
										<input type="submit" name="getUserId" class="adm-btn-save" value="<?=Loc::getMessage("YANDEX_TYRBO_API_SAVE")?>" title="<?=Loc::getMessage("YANDEX_TYRBO_API_SAVE")?>">
									<?endif;?>
								</td>			
							</tr>
							<?if(strlen($arTurboProp['user_id']) > 0):?>
							<tr>
								<th align="right" width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_HOST")?></th>
								<td width="60%">
									...
									<?else:?>	
										<select name="turbo[<?=$val["ID"]?>][host_id]">
											<option value=""><?=Loc::getMessage("YANDEX_TYRBO_API_SELECT")?></option>
