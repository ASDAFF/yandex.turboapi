...
<item turbo="<?=$arField['ITEM']?>">			
	...
	<![CDATA[
							...
							<td width="50%">
								...
								if($arItem['MIN_PRICE'])
								{
									if($arItem['OFFERS'])
									{
										...
										{
											?>
											<p><big><b><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_FROM")?></b></big> <small><del><?=$arItem['MIN_PRICE']['PRINT_VALUE']?></del></small> <big><b><?=$arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></b></big></br><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_DISCOUNT")?> <b><?=$arItem['MIN_PRICE']['PRINT_DISCOUNT']?></b></p>
											<?
										}
										else
										{
											?>
											<p><big><b><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_FROM")?> <?=$arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></b></big></p>
											<?
										}
									}
									else
									{										
										...
										{
											?>
											<p><small><del><?=$arItem['MIN_PRICE']['PRINT_VALUE']?></del></small> <big><b><?=$arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></b></big></br><?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_DISCOUNT")?> <b><?=$arItem['MIN_PRICE']['PRINT_DISCOUNT']?></b></p>
											<?
										}
										else
										{
								...
								if($arItem['ELEMENT']['CATALOG_AVAILABLE'] == 'Y' && $arItem['MIN_PRICE'])
								{
									if($arItem['OFFERS'])
									{
										...
										<button
											...
											data-color="#FFFFFF"
											data-primary="true"><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_MORE")?>
										</button>
										<?
									}
									else
									{
										...
										<button
											...
											data-color="#ffffff"
											data-turbo="false"
											data-primary="true"><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_ADD")?>
										</button>
										<?
									}
								}
								...
								{
									?><p><b><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_AVAILABLE")?></b></p><?
								}
								?>
							</td>
						<?if(!(($k + 1) % 2)):?>
		...
		<button
			...
			data-color="#FFFFFF"
			data-turbo="false"
			data-primary="false"><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_ALL")?>
		</button>
		<p>&nbsp;</p>
		<?
		...
		{
			?>
			<h5><?=Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_SHARE")?></h5>
