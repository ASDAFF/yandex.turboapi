...
<item turbo="<?=$arField['ITEM']?>">			
	...
	?>
	<?
	if(strlen($arField['PUB_DATE']) > 0)
	{
		?>
		<pubDate><?=$arField['PUB_DATE']?></pubDate>
		<?
	}
	?>
	<turbo:content>
	<![CDATA[
		...
		?>
		<?
		if($arField['MIN_PRICE'] && !$arField['OFFERS'])
		{
			if($arField['MIN_PRICE']['DISCOUNT_VALUE'] < $arField['MIN_PRICE']['VALUE'])
			{
				?>
				<p><small><del><?=$arField['MIN_PRICE']['PRINT_VALUE']?></del></small> <big><b><?=$arField['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></b></big></br><?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_DISCOUNT")?> <b><?=$arField['MIN_PRICE']['PRINT_DISCOUNT']?></b></p>
				<?
			}
			else
			{
		...
		if($arField['ELEMENT']['CATALOG_AVAILABLE'] == 'Y' && $arField['MIN_PRICE'])
		{
			if($arField['OFFERS'])
			{
				?>
				<div data-block="accordion">
					...
					foreach($arField["OFFERS"] as $arOffer)
					{
						...
						<div data-block="item" data-title="<?=$arOffer['NAME']?>"<?=($i == 0 ? ' data-expanded="true"' : '')?>>
							...
							{
								echo \Yandex\TurboAPI\Turbo::getStrProrertyValue($arOffer['DISPLAY_PROPERTIES']);
							}
							?>
							<?
							...
							{
								?>
								<p><small><del><?=$arOffer['MIN_PRICE']['PRINT_VALUE']?></del></small> <big><b><?=$arOffer['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></b></big></br><?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_DISCOUNT")?> <b><?=$arOffer['MIN_PRICE']['PRINT_DISCOUNT']?></b></p>
								<?
							}
							else
							{
							...
							<button
								...
								data-color="#FFFFFF"
								data-primary="true"><?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_ADD")?>
							</button>
						</div>
						<?
						$i++;
		...
		{
			?><p><b><?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_AVAILABLE")?></b></p><?
		}
		if(strlen($arField['TURBO_CONTENT']) || strlen($arField['DISPLAY_TURBO_CONTENT_FIELDS']) || $arField['DISPLAY_PROPERTIES'] || $arField['VIDEO'] || $arField['TURBO_CONTENT_FIELDS'])
		{
			?>
			<div data-block="accordion">
				<?
				$expanded = true;
				if(strlen($arField['TURBO_CONTENT']) > 0 || strlen($arField['DISPLAY_TURBO_CONTENT_FIELDS']) > 0)
				{
					$expanded = false;
					?>
					<div data-block="item" data-title="<?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_TAB_1")?>" data-expanded="true">
						<?=$arField['TURBO_CONTENT']?>
						<p>&nbsp;</p>
						<?=$arField['DISPLAY_TURBO_CONTENT_FIELDS']?>
					</div>
					<?
				}
				...
				{
					?>
					<div data-block="item" data-title="<?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_TAB_2")?>" <?=($expanded ? 'data-expanded="true"' : '')?>>
						<?=\Yandex\TurboAPI\Turbo::getStrProrertyValue($arField['DISPLAY_PROPERTIES'])?>
					</div>
					<?
				}
				?>
				...
				{				
					...
					?>
					<div data-block="item" data-title="<?=\Bitrix\Main\Localization\Loc::getMessage("YANDEX_TYRBO_API_TEMP_CATALOG_TAB_4")?><?=($cntVideo > 1 ? '&nbsp;('.$cntVideo.')' : '')?>">
						<?
						if($cntVideo > 1)
						{
							?>
				...
				?>
				<?=\Yandex\TurboAPI\Turbo::getStrAccordionFields($arField['TURBO_CONTENT_FIELDS'])?>
			</div>
			<p>&nbsp;</p>
			<?
		}
