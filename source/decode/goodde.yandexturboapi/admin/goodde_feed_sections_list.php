...
if ($_REQUEST["ACTION"]=="AGENT" && $USER->CanDoOperation('edit_php') && check_bitrix_sessid())
{
	...
	{
		$redirectUrl = "/bitrix/admin/goodde_feed_sections_list.php?lang=".urlencode(LANGUAGE_ID)."&success_export=Y";
...
while ($arRes = $myData->GetNext())
{
	...
	$el_edit_url = htmlspecialcharsbx(\Goodde\YandexTurbo\CGooddeYandexTurboTools::GetAdminElementEditLink($arRes["ID"], 'goodde_feed_sections_edit.php?ID='));
	...
	if ('Y' == $arRes['IN_AGENT'])
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect("/bitrix/admin/goodde_feed_sections_list.php?lang=".LANGUAGE_ID."&".bitrix_sessid_get()."&ACTION=AGENT&FEED_ID=".$arRes['ID']),
		);
	}
	else
	{
...
if (empty($aContext))
{
	$aContext[] = array(
			...
			"LINK" => \Goodde\YandexTurbo\CGooddeYandexTurboTools::GetAdminElementEditLink(0, 'goodde_feed_sections_edit.php?ID='),
			"LINK_PARAM" => "",
