...
use Bitrix\Main\Loader,
	...
	Goodde\YandexTurbo\CGooddeYandexTurboTools;
...
if(!Loader::includeModule('goodde.yandexturboapi')) 
{
	$response = array(
		 'result'  => 'error',
...
$rights = $APPLICATION->GetGroupRight('goodde.yandexturboapi');
if($rights < 'W') 
{
	die('Access denied!');
}
...
function changeIblockTypeId()
{
	...
	$arCatalogs = CGooddeYandexTurboTools::getCatalogs($POST['USE_CATALOG'] == 'Y');
...
function changeIblockId()
{
	...
	$arCatalogSections = CGooddeYandexTurboTools::getCatalogSections($POST['IBLOCK_ID'], $POST['USE_SUBSECTIONS'] == 'Y');
	$cnt               = count($arCatalogSections);
	$attr_size         = ($cnt > 10 ? 10 : $cnt);
	ob_start();
	?>
	<select name="FEED[SECTION_ID][]" size="<?=$attr_size?>" multiple>
		<option value=""<? if(!$POST['SECTION_ID'][0]): ?> selected<? endif ?>><?=Loc::getMessage("GOODDE_TYRBO_API_SELECT_OPTION_EMPTY")?></option>
...
function changeUseCatalog()
{
	...
	?>
	<select name="FEED[SECTION_ID][]" size="<?=$attr_size?>" multiple>
		<option value=""><?=Loc::getMessage("GOODDE_TYRBO_API_SELECT_OPTION_EMPTY")?></option>
...
function changeOfferType()
{
	...
	$contentFields = Loc::getMessage('GOODDE_TYRBO_API_FIELDS');

	// когда запрашивается аяксом кастомное поле-параметр
	if($isCustom) 
	{
	...
	if($iblockId = $POST['IBLOCK_ID']) {
		CGooddeYandexTurboTools::construct();
		CGooddeYandexTurboTools::getIblockInfo($iblockId);
		$useOffers = CGooddeYandexTurboTools::$useOffers;
	}
	...
	<tr class="offer-type-field" data-id="<?$id?>" id="<?=$rowId?>">
		...
		<td class="td-condition">
			<? if($isCustom || $arField['IS_CUSTOM']): ?>
				...
				<input class="accordion_value" type="text" name="FIELDS[CONTENT][<?=$i?>][UNIT_VALUE]" value="<?=$arField['UNIT_VALUE']?>" size="60" placeholder="<?=Loc::getMessage('GOODDE_TYRBO_API_FIELDS_UNIT_PLACEHOLDER')?>">
			<? else: ?>
				...
				<div class="field-row">
					<div class="type_row">
						<select name="FIELDS[CONTENT][<?=$i?>][TYPE][]" onchange="getOfferFieldsSelect(this,'<?=$rowId?>')" class="btn-block">
							<?=CGooddeYandexTurboTools::showFieldTypeSelect($POST['IBLOCK_ID'], $typeId, $typeValue);?>
						</select>
					</div>
					<div class="value_row <?=($typeId == 'NONE' ? 'hide' : '')?>">
						<select name="FIELDS[CONTENT][<?=$i?>][VALUE][]" class="btn-block">
							<?
							if($typeId != 'NONE')
								echo CGooddeYandexTurboTools::showOfferFieldsSelect($POST['IBLOCK_ID'], $typeId, $typeValue);
							?>
						</select>
					</div>
					<div class="controls">
		...
		<td class="td-option">
			<div class="options_block">
				<label>
					...
					$concatValues = (array)Loc::getMessage('GOODDE_TYRBO_API_USE_CONCAT_VALUES');
					?>
					...
					<?=Loc::getMessage('GOODDE_TYRBO_API_USE_CONCAT_LABEL')?> <br>
				...
				<label>
					...
					<?=Loc::getMessage('GOODDE_TYRBO_API_FUNCTION_LABEL')?> <br>
				...
				<label>
					...
					<?=Loc::getMessage('GOODDE_TYRBO_API_TEXT_LENGTH_LABEL')?> <br>
				...
				<label>
					...
					<?=Loc::getMessage('GOODDE_TYRBO_API_DATE_FORMAT_LABEL')?> <br>
				...
				<label>
					...
					<?=Loc::getMessage('GOODDE_TYRBO_API_TEXT_VALUE_LABEL')?> <br>
...
function changeXmlOfferType()
{
	...
	$arOfferType = CGooddeYandexTurboTools::getOfferType($POST['TYPE']);
				...
				<div class="field-row">
					<div class="type_row">
						<select name="FEED[FIELDS][<?=$i?>][TYPE][]" onchange="getOfferFieldsSelect(this,'<?=$rowId?>')" class="btn-block">
							<?=CGooddeYandexTurboTools::showFieldTypeSelect($POST['IBLOCK_ID'], $typeId, $typeValue, true);?>
						</select>
					</div>
					<div class="value_row <?=($typeId == 'NONE' ? 'hide' : '')?>" data-useIproperty="">
						...
						<div class="<?=($typeId == 'IPROPERTY' ? '' : 'hide')?>" data-useIpropertyValue="">
							<input type="text" name="FEED[FIELDS][<?=$i?>][USE_IPROPERTY_VALUE][]" value="<?=$arField['USE_IPROPERTY_VALUE'][$typeKey]?>" <?=($typeId == 'IPROPERTY' ? '' : 'disabled')?> size="44" placeholder="<?=Loc::getMessage('GOODDE_TYRBO_API_USE_IPROPERTY_LABEL')?>">
						</div>
					</div>
					<div class="controls">
						<button type="button" class="adm-btn adm-btn-icon adm-btn-add"></button>
		...
		<td class="td-option">
			<div class="options_block">
				<label>
					...
					<?=Loc::getMessage('GOODDE_TYRBO_API_REQUIRED_LABEL')?>
				</label>
				...
				<label>
					...
					$boolValues = (array)Loc::getMessage('GOODDE_TYRBO_API_BOOLEAN_VALUES');
					?>
					...
					<?=Loc::getMessage('GOODDE_TYRBO_API_BOOLEAN_LABEL')?> <br>
				...
				<label>
					...
					<input id="<?=$rowId?>_use_conditions_checkbox"
						 ...
						 <?=($isCatalog ? '' : 'disabled')?>
						 <?=$useCondition?>>
					<?=Loc::getMessage('GOODDE_TYRBO_API_USE_CONDITIONS_LABEL')?>
				</label>
...
function getOfferFieldsSelect()
{
	...
	ob_start();
	echo CGooddeYandexTurboTools::showOfferFieldsSelect($POST['IBLOCK_ID'], $type);
	$data = ob_get_clean();
	ob_end_clean();
...
function getCatalogCondTree()
{
	...
	$arContentFields = Loc::getMessage('GOODDE_TYRBO_API_FIELDS');
