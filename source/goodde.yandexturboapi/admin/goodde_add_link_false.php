...
if (($_SERVER['REQUEST_METHOD'] == "POST" || $CUR_FILE_POS > 0) && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 1)
	{
		...
		if(strlen($URL_DATA_FILE) > 0)
		{
			...
			{
				$strError .= Loc::getMessage("GOODDE_TYRBO_API_NOT_CSV").".<br>";
			}
			elseif(
				(strlen($FILE_NAME) > 1)
		...
		{
			$strError .= Loc::getMessage("GOODDE_TYRBO_API_NO_DATA_FILE_SIMPLE").".<br>";
		}
	...
	if ($STEP > 2)
	{
		...
		if (strlen($strError) <= 0)
		{
			...
			if (strlen($delimiter_r_char) != 1)
				$strError.= GetMessage("GOODDE_TYRBO_API_NO_DELIMITER")."<br>";
			...
			if (strlen($strError) <= 0)
			{
				...
				{
					$strError.= GetMessage("GOODDE_TYRBO_API_NO_DATA")."<br>";
				}
	...
	if ($STEP > 3)
	{
		...
		if (!$bFieldsPres)
			$strError.= GetMessage("GOODDE_TYRBO_API_NO_FIELDS")."<br>";
		...
		if (strlen($strError) > 0)
		{
			if(strlen($runError) <= 0)
			{
				$strError.= GetMessage("GOODDE_TYRBO_API_TOTAL_ERRS")." ".$error_lines.".<br>";
				$strError.= GetMessage("GOODDE_TYRBO_API_TOTAL_COR1")." ".$correct_lines." ".GetMessage("GOODDE_TYRBO_API_TOTAL_COR2")."<br>";
			}
			$STEP = 3;
		}
		//*****************************************************************//
...
/////////////////////////////////////////////////////////////////////
$APPLICATION->SetTitle(GetMessage("GOODDE_TYRBO_API_PAGE_TITLE").$STEP);
require ($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
/*********************************************************************/
/********************  BODY  *****************************************/
/*********************************************************************/
...
if(!$bAllLinesLoaded)
{
	...
	<?echo GetMessage("GOODDE_TYRBO_API_AUTO_REFRESH"); ?>
	<a href="<?echo $APPLICATION->GetCurPage(); ?>?lang=<?echo LANGUAGE_ID; ?>&<?echo $strParams ?>"><?echo GetMessage("GOODDE_TYRBO_API_AUTO_REFRESH_STEP"); ?></a><br>
...
if ($STEP == 1)
{
	CAdminMessage::ShowMessage(array('MESSAGE' => Loc::getMessage("GOODDE_TYRBO_API_HELP_INFO"), 'TYPE' => 'OK', 'HTML' => true));
	CAdminMessage::ShowMessage(array('MESSAGE' => Loc::getMessage("GOODDE_TYRBO_API_HELP_INFO_2"), 'TYPE' => 'ERROR', 'HTML' => true));
}
?>
...
<?$aTabs = array(
	array(
		"DIV" => "edit1",
		"TAB" => GetMessage("GOODDE_TYRBO_API_TAB1") ,
		"ICON" => "",
		"TITLE" => GetMessage("GOODDE_TYRBO_API_TAB1_ALT"),
	) ,
	array(
		"DIV" => "edit2",
		"TAB" => GetMessage("GOODDE_TYRBO_API_TAB2") ,
		"ICON" => "",
		"TITLE" => GetMessage("GOODDE_TYRBO_API_TAB2_ALT"),
	) ,
	array(
		"DIV" => "edit3",
		"TAB" => GetMessage("GOODDE_TYRBO_API_TAB3") ,
		"ICON" => "",
		"TITLE" => GetMessage("GOODDE_TYRBO_API_TAB3_ALT"),
	) ,
	array(
		"DIV" => "edit4",
		"TAB" => GetMessage("GOODDE_TYRBO_API_TAB4") ,
		"ICON" => "",
		"TITLE" => GetMessage("GOODDE_TYRBO_API_TAB4_ALT"),
	) ,
);
...
if ($STEP == 1)
{
	?>
	<tr>
		<td width="40%"><?=Loc::getMessage("GOODDE_TYRBO_API_FILE_NAME")?></td>
		<td width="60%">
			<input type="text" name="URL_DATA_FILE" value="<?echo htmlspecialcharsbx($URL_DATA_FILE); ?>" size="30">
			<input type="button" value="<?=Loc::getMessage("GOODDE_TYRBO_API_OPEN");?>" OnClick="BtnClick()">
			<?
...
?>
	...
	<tr>
		<td class="adm-detail-valign-top"><?echo GetMessage("GOODDE_TYRBO_API_RAZDEL_TYPE"); ?>:</td>
		<td>
			<input type="radio" name="delimiter_r" id="delimiter_r_TZP" value="TZP" <?
			if ($delimiter_r == "TZP" || strlen($delimiter_r) <= 0)
				echo "checked" ?>><label for="delimiter_r_TZP"><?echo GetMessage("GOODDE_TYRBO_API_TZP"); ?></label><br>
					<input type="radio" name="delimiter_r" id="delimiter_r_ZPT" value="ZPT" <?
			if ($delimiter_r == "ZPT")
				echo "checked" ?>><label for="delimiter_r_ZPT"><?echo GetMessage("GOODDE_TYRBO_API_ZPT"); ?></label><br>
					<input type="radio" name="delimiter_r" id="delimiter_r_TAB" value="TAB" <?
			if ($delimiter_r == "TAB")
				echo "checked" ?>><label for="delimiter_r_TAB"><?echo GetMessage("GOODDE_TYRBO_API_TAB"); ?></label><br>
					<input type="radio" name="delimiter_r" id="delimiter_r_SPS" value="SPS" <?
			if ($delimiter_r == "SPS")
				echo "checked" ?>><label for="delimiter_r_SPS"><?echo GetMessage("GOODDE_TYRBO_API_SPS"); ?></label><br>
					<input type="radio" name="delimiter_r" id="delimiter_r_OTR" value="OTR" <?
			if ($delimiter_r == "OTR")
				echo "checked" ?>><label for="delimiter_r_OTR"><?echo GetMessage("GOODDE_TYRBO_API_OTR"); ?></label>
			<input type="text" name="delimiter_other_r" size="3" value="<?echo htmlspecialcharsbx($delimiter_other_r); ?>">
		</td>
	</tr>
	<tr>
		<td><?echo GetMessage("GOODDE_TYRBO_API_FIRST_NAMES"); ?>:</td>
		<td>
	...
	<tr class="heading">
		<td colspan="2"><?echo GetMessage("GOODDE_TYRBO_API_DATA_SAMPLES"); ?></td>
	</tr>
	<tr>
...
?>
	<tr class="heading">
		<td colspan="2"><?echo GetMessage("GOODDE_TYRBO_API_FIELDS_SOOT"); ?></td>
	</tr>
	<?
	$arAvailFields = array();
	...
	foreach ($arDataFileFields as $i => $field)
	{
		...
		<tr>
			<td width="40%">
				<b><?echo GetMessage("GOODDE_TYRBO_API_FIELD"); ?> <?echo $i + 1 ?></b> (<?echo htmlspecialcharsbx($field); ?>):
			</td>
			<td width="60%">
	...
	?>
	<tr class="heading">
		<td colspan="2"><?echo GetMessage("GOODDE_TYRBO_API_ADDIT_SETTINGS"); ?></td>
	</tr>
	<tr>
		<td class="adm-detail-valign-top"><?echo GetMessage("GOODDE_TYRBO_API_LID"); ?>:</td>
		<td align="left">
			<?\Goodde\YandexTurbo\CGooddeYandexTurboTools::ShowLidField('LID', htmlspecialcharsbx($LID));?>
		</td>
	</tr>
...
?>
	<tr>
		...
		<? CAdminMessage::ShowMessage(array(
			"TYPE" => "PROGRESS",
			"MESSAGE" => !$bAllLinesLoaded? GetMessage("GOODDE_TYRBO_API_AUTO_REFRESH_CONTINUE"): GetMessage("GOODDE_TYRBO_API_SUCCESS"),
			"DETAILS" =>

			GetMessage("GOODDE_TYRBO_API_SU_ALL").' <b>'.$line_num.'</b><br>'
			.GetMessage("GOODDE_TYRBO_API_SU_CORR").' <b>'.$correct_lines.'</b><br>'
			.GetMessage("GOODDE_TYRBO_API_SU_ER").' <b>'.$error_lines.'</b><br>',
			"HTML" => true,
		))?>
		</td>
	</tr>
...
if ($STEP < 4)
{
	...
	{
		?>
		<input type="submit" name="backButton" value="&lt;&lt; <?echo GetMessage("GOODDE_TYRBO_API_BACK"); ?>">
		<?
	}
	?>
	<input type="submit" value="<?echo ($STEP == 3) ? GetMessage("GOODDE_TYRBO_API_NEXT_STEP_F") : GetMessage("GOODDE_TYRBO_API_NEXT_STEP"); ?> &gt;&gt;" name="submit_btn" class="adm-btn-save">
	<?
}
else
{
	?>
	<input type="submit" name="backButton2" value="&lt;&lt; <?echo GetMessage("GOODDE_TYRBO_API_2_1_STEP"); ?>" class="adm-btn-save">
	<?
}
$tabControl->End();
?>
