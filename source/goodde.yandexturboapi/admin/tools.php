...
class CGooddeYandexTurboTools
{
	...
	public static function getCurrency()
	{
		if(Loader::includeModule('currency')) {
			...
			$currencies = Loc::getMessage('GOODDE_TYRBO_API_CURRENCIES');
		}
	...
	{
		return Loc::getMessage('GOODDE_TYRBO_API_CURRENCY_RATES');
	}

	// Price types
	public static function getPriceTypes()
	{
		$types = (array)Loc::getMessage('GOODDE_TYRBO_API_OPTIMAL_RICE');
	...
	public static function getHttpFilePath($path)
	{
		if($path)
			...
			$path = Loc::getMessage('GOODDE_TYRBO_API_DEFAULT_FILE_PATH');
	...
	public static function getOfferFieldsSelect($iblockId)
	{
		...
		// Element Fields
		$arFields['FIELDS'] = Loc::getMessage('GOODDE_TYRBO_API_OFFER_FIELDS_LANG');

		// Fields offers
		$arFields['OFFER_FIELD'] = Loc::getMessage('GOODDE_TYRBO_API_OFFER_FIELDS_LANG');
		...
		// Element Fields
		$arFields["PRODUCT"] = Loc::getMessage('GOODDE_TYRBO_API_CATALOG_FIELDS_LANG');

		// Price
		$arFields['PRICE'] = Loc::getMessage('GOODDE_TYRBO_API_PRICE_FIELDS_LANG');

		//Currency
		$arFields['CURRENCY'] = static::getCurrency();

		// Meta-tag
		$arFields['IPROPERTY'] = Loc::getMessage('GOODDE_TYRBO_API_IPROPERTY_FIELDS_LANG');
	...
	{
		$arOptions   = array();
		$arOptions[] = '<option value="">' . Loc::getMessage('GOODDE_TYRBO_API_SELECT_OPTION_EMPTY') . '</option>';
	...
	public static function showFieldTypeSelect($iblockId, $typeId, $typeVal, $isXmlProfile = false)
	{
		...
		if($isXmlProfile)
		{
			$arType = (array)Loc::getMessage('GOODDE_TYRBO_API_FIELD_TYPE_SELECT_XML_PROFILE');
		}
		else
		{
			$arType = (array)Loc::getMessage('GOODDE_TYRBO_API_FIELD_TYPE_SELECT');
		}
