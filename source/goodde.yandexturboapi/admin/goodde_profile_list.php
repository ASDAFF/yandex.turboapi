...
$sTableId = "goodde_yandex_turbo_profile";
...
if($USER->CanDoOperation('edit_php') && check_bitrix_sessid())
{
	...
	if($PROFILE_ID > 0)
	{
		if($_REQUEST['action'] === 'agent')
		{
			...
			if($arProfile['IN_AGENT'] == 'Y')
				CAgent::RemoveAgent("\Goodde\Export\ProfileTools::preGenerateExport(".$PROFILE_ID.");", $moduleId);
			else
				CAgent::AddAgent("\Goodde\Export\ProfileTools::preGenerateExport(".$PROFILE_ID.");", $moduleId, "N", $agentPeriod*60*60, "", "Y");
		...
		elseif($_REQUEST['action_button_goodde_yandex_turbo_profile'] === 'delete')
		{
			$DB->StartTransaction();
			...
			{
				$DB->Rollback();
				$lAdmin->AddGroupError(Loc::getMessage("GOODDE_TYRBO_API_DELETE_ERROR"), $PROFILE_ID);
			}
			$DB->Commit();
		}
	}
	...
	if($isRedirect)
	{
		$redirectUrl = "/bitrix/admin/goodde_profile_list.php?lang=".urlencode(LANGUAGE_ID)."&success_export=Y";
...
$query = \Goodde\Export\TurboProfileTable::query();
...
$lAdmin->SetNavigationParams($dbResultList, array("BASE_LINK" => "/bitrix/admin/goodde_profile_list.php"));

$columns = \Goodde\Export\TurboProfileTable::getEntity()->getFields();
...
{
	$el_edit_url = htmlspecialcharsbx(\Goodde\YandexTurbo\CGooddeYandexTurboTools::GetAdminElementEditLink($arProfile["ID"], 'goodde_profile_edit.php?ID='));
	...
	$arActions = Array();
	$arActions[] = array(
		"TEXT" => Loc::getMessage("GOODDE_TYRBO_API_EDIT"), 
		"ACTION" => $lAdmin->ActionRedirect($el_edit_url), 
		"DEFAULT" => true
	);
	$arActions[] = array(
		"TEXT" => GetMessage("GOODDE_TYRBO_API_COPY_PROFILE"),
		"TITLE" => GetMessage("GOODDE_TYRBO_API_COPY_PROFILE"),
	...
	if ('Y' == $arProfile['IN_AGENT'])
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect("/bitrix/admin/goodde_profile_list.php?lang=".LANGUAGE_ID."&".bitrix_sessid_get()."&action=agent&PROFILE_ID=".$arProfile['ID']),
		);
	}
	else
	{
	...
	if($POST_RIGHT >= "W")
		$arActions[] = array("ICON" => "delete", "TEXT" => Loc::getMessage("GOODDE_TYRBO_API_DELETE"), "ACTION" => "if(confirm('" . GetMessageJS("GOODDE_TYRBO_API_DEL_CONF") . "')) " . $lAdmin->ActionDoGroup($arProfile["ID"], "delete"));
...
$arContext[] = array(
	...
	"LINK" => \Goodde\YandexTurbo\CGooddeYandexTurboTools::GetAdminElementEditLink(0, 'goodde_profile_edit.php?ID='),
);

$lAdmin->AddAdminContextMenu($arContext, false);
$lAdmin->CheckListMode();

$APPLICATION->SetTitle(GetMessage("GOODDE_TYRBO_API_ADMIN_TITLE"));
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
...
echo BeginNote();
	...
	<?else:?>
		...
		<?=Loc::getMessage("GOODDE_TYRBO_API_NOTES11_EXT", array('#FILE#' => '/bitrix/php_interface/include/yandex_turbo/cron_frame_xml.php'));?><br>
