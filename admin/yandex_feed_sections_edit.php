...
$aTabs = array(
	...
	array("DIV" => "edit2", "TAB" => Loc::getMessage("YANDEX_TYRBO_API_TAB_SECTION_FILTER_TITLE"), "ICON"=>"main_user_edit", "TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_TAB_SECTION_FILTER_TITLE")),
	...
	array("DIV" => "edit4", "TAB" => Loc::getMessage("YANDEX_TYRBO_API_TAB_DEBUG"), "ICON"=>"main_user_edit", "TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_TAB_DEBUG_TITLE")),
	array("DIV" => "edit5", "TAB" => Loc::getMessage("YANDEX_TYRBO_API_TAB_PRODUCTION"), "ICON"=>"main_user_edit", "TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_TAB_PRODUCTION_TITLE")),
);
...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && check_bitrix_sessid())
{
	...
	{
		if ($apply != "")
		  LocalRedirect("/bitrix/admin/yandex_feed_sections_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
		  LocalRedirect("/bitrix/admin/yandex_feed_sections_list.php?lang=".LANG);
	}
	else
	{
...
$arPubDate = array(
    'DATE_CREATE' => Loc::getMessage("YANDEX_TYRBO_API_DATE_CREATE"),
    'TIMESTAMP_X' => Loc::getMessage("YANDEX_TYRBO_API_TIMESTAMP_X"),
    'ACTIVE_FROM' => Loc::getMessage("YANDEX_TYRBO_API_ACTIVE_FROM"),
);
$arShareNetworks = array(
    'facebook' => 'Facebook',
    'google' => 'Google+',
    'odnoklassniki' => Loc::getMessage("YANDEX_TYRBO_API_OK"),
    'telegram' => 'Telegram',
    'twitter' => 'Twitter',
    'vkontakte' => Loc::getMessage("YANDEX_TYRBO_API_VK"),
);
...
{
    $arFields['FIGCAPTION_VIDEO'] = Loc::getMessage('YANDEX_TYRBO_API_FIGCAPTION_VIDEO_DEFAULT');
}
...
if($ID <= 0)
{
	...
	{
		$arFields['MENU'][] = array(
			0 => Loc::getMessage("YANDEX_TYRBO_API_MENU_DEFAULT"),
			1 => '/',
		);
	}
}
...
if($IBLOCK_ID > 0) 
{
	$arSections = \Yandex\TurboAPI\CYandexTurboAPITools::getCatalogSections($IBLOCK_ID, true);
	$arTemplateList = \Yandex\TurboAPI\Turbo::getTemplateList('section');
	$ipropTemlates = new \Bitrix\Iblock\InheritedProperty\IblockTemplates($IBLOCK_ID);
	...
	$arAscDesc = array(
		'asc' => Loc::getMessage("YANDEX_TYRBO_API_SORT_ASC"),
		'desc' => Loc::getMessage("YANDEX_TYRBO_API_SORT_DESC"),
	);
}
...
if($bVarsFromForm)
  $DB->InitTableVarsForEdit("yandex_yandex_turbo_feed", "", "str_");
...
$aMenu = array(
	array(
		...
		"TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_LIST_TITLE"),
		"LINK"=>"yandex_feed_sections_list.php?lang=".LANG,
		"ICON"=>"btn_list",
	)
);
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"yandex_feed_sections_edit.php?lang=".LANG,
		"ICON"=>"btn_new",
	);
	$aMenu[] = array(
		"TEXT"=>Loc::getMessage("YANDEX_TYRBO_API_DELETE"),
		"TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_DELETE"),
		"LINK"=>"javascript:if(confirm('".Loc::getMessage("YANDEX_TYRBO_API_DELETE_CONF")."'))window.location='yandex_feed_sections_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
	...
	<div class="form-block <?=($key == 'template' ? 'template' : '')?>" data-form-block="" <?=($key == 'template' ? 'data-form-block-template=""' : '')?>>
		<span>
			<select name="<?=($key != 'template' ? 'FEEDBACK[TYPE]['.$key.'][STICK]' : '')?>">
				<option <?=($arFeedback['STICK'] == 'left' ? 'selected=""' : '')?> value="left"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_LEFT")?></option>
				<option <?=($arFeedback['STICK'] == 'right' ? 'selected=""' : '')?> value="right"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_RIGHT")?></option>
				<option <?=($arFeedback['STICK'] == 'false' ? 'selected=""' : '')?> value="false"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_FALSE")?></option>
			</select>
		</span>
		<span>
			<select name="<?=($key != 'template' ? 'FEEDBACK[TYPE]['.$key.'][PROVIDER_KEY]' : '')?>" data-similar-init="">
				<option <?=($arFeedback['PROVIDER_KEY'] == 'call' ? 'selected=""' : '')?> value="call"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_CALL")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'chat' ? 'selected=""' : '')?> value="chat"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_CHAT")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'mail' ? 'selected=""' : '')?> value="mail"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_MAIL")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'callback' ? 'selected=""' : '')?> value="callback"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_CALLBACK")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'facebook' ? 'selected=""' : '')?> value="facebook"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_FACEBOOK")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'google' ? 'selected=""' : '')?> value="google"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_GOOGLE")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'odnoklassniki' ? 'selected=""' : '')?> value="odnoklassniki"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_ODNOKLASSNIKI")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'telegram' ? 'selected=""' : '')?> value="telegram"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_TELEGRAM")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'twitter' ? 'selected=""' : '')?> value="twitter"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_TWITTER")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'viber' ? 'selected=""' : '')?> value="viber"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_VIBER")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'vkontakte' ? 'selected=""' : '')?> value="vkontakte"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_VKONTAKTE")?></option>
				<option <?=($arFeedback['PROVIDER_KEY'] == 'whatsapp' ? 'selected=""' : '')?> value="whatsapp"><?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_WHATSAPP")?></option>
			</select> 
		</span>
		...
		<span class="form-block-remove"><label title="<?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_DELETE")?>"><img src="/bitrix/themes/.default/images/actions/delete_button.gif" alt="<?=GetMessage("YANDEX_TYRBO_API_FEEDBACK_STICK_OP_DELETE")?>"><input data-remove="<?=($key != 'template' ? 'static' : '')?>" type="checkbox" name="<?=($key != 'template' ? 'FEEDBACK[TYPE]['.$key.'][DELETE]' : '')?>"></label></span>
	</div>
	<?
}
...
<form id="feed_edit_form" method="POST" action="<?echo $APPLICATION->GetCurPage()?>" enctype="multipart/form-data" name="feed_edit_form">
	...
	if($ID <= 0)
	{
		?>
		<tr>
			<th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_IBLOCK") ?></th>
			<td width="60%">
				<select name="IBLOCK_ID" required id="select-iblock">
					<option value=""><?=Loc::getMessage("YANDEX_TYRBO_API_VYBRAT")?></option>
	...
	if($IBLOCK_ID > 0)
	{
		...
		?>
		<tr>
            <th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_LID")?></th>
            <td width="60%">
				<?\Yandex\TurboAPI\CYandexTurboAPITools::ShowLidField('LID', $arFields['LID']);?>
            </td>
        </tr>
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_ACTIVE")?></td>
            <td>
                <input type="checkbox" name="ACTIVE" value="Y" <?=($arFields['ACTIVE'] ? ($arFields['ACTIVE'] == 'Y' ? 'checked' : '') : 'checked')?>>
            </td>
        </tr>
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_SORT")?></td>
            <td>
                <input type="text" size="10" name="SORT" value="<?=$arFields['SORT']?>">
            </td>
        </tr>
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_TEMPLATE")?></td>
            <td>
		...
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_ITEM_STATUS")?></td>
            <td>
				<select name="ITEM_STATUS" style="width:45%">
					<option value=""><?=Loc::getMessage("YANDEX_TYRBO_API_NOT_CHOSEN")?></option>
					<option value="false" <?=($arFields['ITEM_STATUS'] == 'false' ? 'selected' : '')?>><?=Loc::getMessage("YANDEX_TYRBO_API_ITEM_STATUS_FALSE")?></option>
					<option value="true" <?=($arFields['ITEM_STATUS'] == 'true' ? 'selected' : '')?>><?=Loc::getMessage("YANDEX_TYRBO_API_ITEM_STATUS_TRUE")?></option>
				</select>
            </td>
        </tr>
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_ALL_FEED")?></td>
            <td>
				<input type="checkbox" name="ALL_FEED" id="ALL_FEED" value="Y" <?=($arFields['ALL_FEED'] == 'Y' ? 'checked="checked"' : '')?>>
            </td>
        </tr>
		<tr>
            <th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_LIMIT")?></th>
            <td width="60%">
                <input type="text" size="10" name="LIMIT" id="LIMIT" value="<?=$arFields['LIMIT']?>">
            </td>
        </tr>
		<tr>
            <th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_AMOUNT_ITEM")?></th>
            <td width="60%">
		...
		if($ID > 0)
		{
			?>
			<tr>
				<th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_SERVER_ADDRESS") ?></th>
				<td width="60%">
					<?=$arFields['SERVER_ADDRESS']?>
				</td>
			</tr>
			<tr>
				<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_IS_DOMAIN") ?></td>
				<td width="60%">
			...
			<tr>
				<td colspan="2" align="center">
					<?
					echo BeginNote();
						echo Loc::getMessage("YANDEX_TYRBO_API_SUBDOMAIN_NOTE");
					echo EndNote();
					?>
				</td>
			</tr>
			<tr data-optioncode="HOST_ID_SUBDOMAIN">
				<th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_HOST_ID_SUBDOMAIN") ?></th>
				<td width="60%">
					...
					<?else:?>	
						<select name="FIELDS[HOST_ID_SUBDOMAIN]" id="HOST_ID_SUBDOMAIN">
							<option value=""><?=Loc::getMessage("YANDEX_TYRBO_API_VYBRAT")?></option>
							<?foreach(\Yandex\TurboAPI\Model\Request::curHost($arFields['LID']) as $arHost):?>
		...
		?>
		<tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_8") ?></td>
        </tr>
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_SECTION_URL")?></td>
            <td>
		...
		if($ID > 0)
		{
			?>
			<tr>
				<td><?=Loc::getMessage("YANDEX_TYRBO_API_IPROPERTY_VALUES_TITLE")?></td>
				<td><?echo IBlockInheritedPropertyInput($IBLOCK_ID, "G_SECTION_META_TITLE_".$ID, $str_IPROPERTY_TEMPLATES, "S")?></td>
			</tr>
			<tr>
				<td><?=Loc::getMessage("YANDEX_TYRBO_API_IPROPERTY_VALUES_PAGE_TITLE")?></td>
		...
		?>
        <tr>
            <th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_CONTENT") ?></th>
            <td width="60%">
                <select name="CONTENT" id="CONTENT" style="width:45%">
                    <optgroup label="<?=Loc::getMessage("YANDEX_TYRBO_API_FILED_IBLOCK_SECTION") ?>">
                        <option value="DESCRIPTION" <?= ($arFields['CONTENT'] == 'DESCRIPTION' ? 'selected' : ''); ?>>
                            <?=Loc::getMessage("YANDEX_TYRBO_API_SECTION_DESCRIPTION") ?></option>
                    </optgroup>
                    <?if(!empty($arPropUserFields)):?>
                        <optgroup label="<?=Loc::getMessage("YANDEX_TYRBO_API_UF_PROPERTY_IBLOCK_SECTION") ?>">
		...
		<tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_SECTION_USER_FIELDS_TITLE")?></td>
            <td width="60%">
		...
		<tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_9") ?></td>
        </tr>
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_DETAIL_URL")?></td>
            <td>
		...
		if(!empty($arPrice))
		{
			?>
			<tr>
				<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_PRICE")?></td>
				<td width="60%">
		...
		?>
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_SORT")?></td>
			<td width="60%">
				<select onChange="setSortProp(this)" data-sort-prop="true" name="FIELDS[SORT][ELEMENT_SORT_FIELD]" id="ELEMENT_SORT_FIELD" style="width:45%">
					<option value=""><?=Loc::getMessage("YANDEX_TYRBO_API_SORT_OTHER")?></option>
		...
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_ELEMENT_SORT_ORDER")?></td>
			<td width="60%">
		...
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_SORT2")?></td>
			<td width="60%">
		...
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_ELEMENT_SORT_ORDER2")?></td>
			<td width="60%">
		...
		<tr>
            <th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_PAGE_ELEMENT_COUNT")?></th>
            <td width="60%">
                <input type="text" size="10" name="FIELDS[PAGE_ELEMENT_COUNT]" id="PAGE_ELEMENT_COUNT" value="<?=$arFields['FIELDS']['PAGE_ELEMENT_COUNT']?>">
            </td>
        </tr>		
        <tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED") ?></td>
        </tr>
        <tr>
            <th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_NAME") ?></th>
            <td width="60%">
                <input type="text" size="60" name="NAME" id="NAME" value="<?= $arFields['NAME']; ?>">
            </td>
        </tr>
		<tr>
            <td><?=Loc::getMessage("YANDEX_TYRBO_API_FIGCAPTION_VIDEO")?></td>
            <td>
                <input type="text" size="60" name="FIGCAPTION_VIDEO" id="FIGCAPTION_VIDEO" value="<?=$arFields['FIGCAPTION_VIDEO']?>">
            </td>
        </tr>
		<tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_2") ?></td>
        </tr>
		<tr>
			<td colspan="2">
					...
					<tr>
						<td align="center">
							<table cellPadding="2" cellSpacing="2" border="0" width="60%" id="t" class="internal">
								<tr class="heading">
									<td align="center" width="35%"><?=Loc::getMessage("YANDEX_TYRBO_API_MENU_NAME")?></td>
									<td align="center" width="35%"><?=Loc::getMessage("YANDEX_TYRBO_API_MENU_LINK")?></td>
									<td align="center" width="20%"><?=Loc::getMessage("YANDEX_TYRBO_API_MENU_SORT")?></td>
									<td align="center" width="10%"><?=Loc::getMessage("YANDEX_TYRBO_API_MENU_DEL")?></td>
								</tr>
								<?								
								$itemcnt = 0;
								for($i = 1, $l = count($arFields['MENU']); $i <= $l; $i++)
								{
									...
									<tr id="<?= $i?>">
										<td align="right" colspan="4"><input type="button" onClick="AddMenuItem(this)" value="<?=GetMessage("YANDEX_TYRBO_API_MENU_ADD_ITEM")?>"></td>
									</tr>
									<?
								}
								?>
		...
		<tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_3") ?></td>
        </tr>
		 <tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_GALLERY_TITLE")?></td>
            <td width="60%">
		...
		<tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_RELATED_SOURCE") ?></td>
            <td width="60%">
                <select name="FIELDS[SECTION_GALLERY]" id="SECTION_GALLERY" style="width:45%">
                    <option value=""><?=Loc::getMessage("YANDEX_TYRBO_API_NOT_CHOSEN")?></option>
                    <?if(!empty($arGalleryProps)):?>
                        <optgroup label="<?=Loc::getMessage("YANDEX_TYRBO_API_PROPERTY_TYPE_FILE") ?>">
        ...
        <tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_4")?></td>
        </tr>
        <tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_RELATED_SOURCE")?></td>
            <td width="60%">
                <select name="RELATED_SOURCE" id="RELATED_SOURCE" style="width:45%">
                    <option value="QUEUE" <?=($arFields['RELATED_SOURCE'] == 'QUEUE' ? 'selected' : '')?>><?=Loc::getMessage("YANDEX_TYRBO_API_QUEUE")?></option>
                    <option value="AUTOPARSING" <?=($arFields['RELATED_SOURCE'] == 'AUTOPARSING' ? 'selected' : '')?>><?=Loc::getMessage("YANDEX_TYRBO_API_AUTO_SPARSING")?></option>
                </select>
            </td>
        </tr>
        <tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_RELATED_LIMIT")?></td>
            <td width="60%">
                <input type="text" size="10" name="RELATED_LIMIT" id="RELATED_LIMIT"  value="<?=$arFields['RELATED_LIMIT']?>">
            </td>
        </tr>
        <tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_SHARE_BUTTON")?></td>
        </tr>
        <tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_SERVICES")?></td>
            <td width="60%">
		...
		<tr class="heading">
		  <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_5")?></td>
		</tr>
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_RELATED_FORM_AGREEMENT_COMPANY")?>:</td>
			<td width="60%">
				<input type="text" name="FORM[AGREEMENT][COMPANY]" value="<?=$arFields['FORM']['AGREEMENT']['COMPANY']?>" style="width:50%">
			</td>
		</tr>
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_RELATED_FORM_AGREEMENT_LINK")?>:</td>
			<td width="60%">
		...
		<tr class="heading">
		  <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_6")?></td>
		</tr>
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_FEEDBACK_SHOW")?>:</td>
			<td width="60%">
		...
		<tr>
			<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_FEEDBACK_TITLE")?>:</td>
			<td width="60%">
				<input type="text" name="FEEDBACK[TITLE]" value="<?=$arFields['FEEDBACK']['TITLE']?>" style="width:50%">
			</td>
		</tr>				
		<tr>
			<td colspan="2" style="padding: 15px 15px 3px;text-align: center; color: #4B6267;font-weight: bold; border-bottom: 5px solid #E0E8EA;"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_7")?></td>
		</tr>
		<tr>
			<td width="40%"></td>
			<td width="60%" data-clone-container="">
				...
				<div class="add-more" ><span class="adm-btn" data-add-more=""><span><?=Loc::getMessage("YANDEX_TYRBO_API_FEEDBACK_TYPE_ADD")?></span></span></div>
			</td>
		</tr>
		<?
	}
	...
	<tr class="heading" align="center">
		<td colspan="2"><?=Loc::getMessage('YANDEX_TYRBO_API_SECTION_FILTER')?></td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage('YANDEX_TYRBO_API_SECTIONS_FILTER_ACTIVE')?></td>
		<td width="60%">
			<input name="FILTER[SECTIONS][ACTIVE]" value="Y" type="checkbox"<?=($arFields['SECTIONS_FILTER']['ACTIVE'] == 'Y' ? ' checked' : '')?>>
		</td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage('YANDEX_TYRBO_API_SECTIONS_FILTER_GLOBAL_ACTIVE')?></td>
		<td width="60%">
			<input name="FILTER[SECTIONS][GLOBAL_ACTIVE]" value="Y" type="checkbox"<?=($arFields['SECTIONS_FILTER']['GLOBAL_ACTIVE'] == 'Y' ? ' checked' : '')?>>
		</td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_SECTION_FILTER_SECTIONS_ID")?></td>
		<td width="60%">
	...
	?>
	<tr class="heading" align="center">
		<td colspan="2"><?=Loc::getMessage('YANDEX_TYRBO_API_ELEMENTS_FILTER')?></td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage('YANDEX_TYRBO_API_FILTER_ACTIVE')?></td>
		<td width="60%">
			<input name="FILTER[ELEMENTS][ACTIVE]" value="Y" type="checkbox"<?=($arFields['ELEMENTS_FILTER']['ACTIVE'] == 'Y' ? ' checked' : '')?>>
		</td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_FILTER_ACTIVE_DATE")?></td>
		<td width="60%">
			<input type="checkbox" name="FILTER[ELEMENTS][ACTIVE_DATE]" value="Y" <?=($arFields['ELEMENTS_FILTER']['ACTIVE_DATE'] == 'Y' ? 'checked' : '')?>>
		</td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage('YANDEX_TYRBO_API_ELEMENTS_FILTER_SECTION_ACTIVE')?></td>
		<td width="60%">
			<input name="FILTER[ELEMENTS][SECTION_ACTIVE]" value="Y" type="checkbox"<?=($arFields['ELEMENTS_FILTER']['SECTION_ACTIVE'] == 'Y' ? ' checked' : '')?>>
		</td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage('YANDEX_TYRBO_API_ELEMENTS_FILTER_SECTION_GLOBAL_ACTIVE')?></td>
		<td width="60%">
	...
	<tr>
		<td width="40%"><?=Loc::getMessage('YANDEX_TYRBO_API_ELEMENTS_FILTER_INCLUDE_SUBSECTIONS')?></td>
		<td width="60%">
	...
	if($catalogIncluded && $arCatalog['CATALOG'] == 'Y')
	{
		?>
		<tr>
			<td width="40%"><span id="hint_elements_catalog_available"></span>&nbsp;<?=Loc::getMessage('YANDEX_TYRBO_API_FILTER_CATALOG_AVAILABLE')?></td>
			<td width="60%">
		...
		if($arCatalog['CATALOG_TYPE'] == \CCatalogSku::TYPE_FULL || $arCatalog['CATALOG_TYPE'] == \CCatalogSku::TYPE_PRODUCT)
		{
			?>
			<tr class="heading" align="center">
				<td colspan="2"><?=Loc::getMessage('YANDEX_TYRBO_API_OFFERS_FILTER')?></td>
			</tr>
			<tr>
				<td width="40%"><?=Loc::getMessage('YANDEX_TYRBO_API_FILTER_ACTIVE')?></td>
				<td width="60%">
			...
			<tr>
				<td width="40%"><span id="hint_offers_catalog_available"></span>&nbsp;<?=Loc::getMessage('YANDEX_TYRBO_API_FILTER_CATALOG_AVAILABLE')?></td>
				<td width="60%">
	...
	$tabControl->BeginNextTab();
	?>
	<input class="adm-btn-save" value="<?=Loc::getMessage("YANDEX_TYRBO_API_RUN")?>" onclick="generateTurboFeed('<?=$ID?>', 'debug_sections_run')" name="debug_run" id="feed_debug_sections_run_button_<?=$ID?>" type="button">
	<?
	$tabControl->BeginNextTab();
	?>
	<input class="adm-btn-save" value="<?=Loc::getMessage("YANDEX_TYRBO_API_RUN")?>" onclick="generateTurboFeed('<?=$ID?>', 'production_sections_run')" name="production_run" id="feed_production_run_button_<?=$ID?>" type="button">
	<?
	$tabControl->Buttons(
	  array(
		"disabled"=>false,
		"back_url"=>"yandex_feed_sections_list.php?lang=".LANG,
...
<?
$tabControl->ShowWarnings("yandex_feed_sections_edit", $message);
?>
<script>
...
BX.finishTurboFeed = function(ID)
{
	if(ID > 0){
		...
		document.location = "/bitrix/admin/yandex_task_edit.php?ID="+ID+"&lang=<?=LANGUAGE_ID?>";
	}
	else{
		tabControl.SelectTab("edit4");
		document.location = "/bitrix/admin/yandex_task_list.php?lang=<?=LANGUAGE_ID?>";
	}
};
...
function AddMenuItem(ob)
{
	...
	oCell.align='right';
	oCell.colSpan="4";
	oCell.innerHTML = '<input type="button" onClick="AddMenuItem(this)" value="<?=GetMessage("YANDEX_TYRBO_API_MENU_ADD_ITEM")?>">';
...
<div id="turbo_feed_run" style="display: none;">
	<div id="turbo_feed_progress"><?=Turbo::showProgress(Loc::getMessage('YANDEX_RUN_INIT'), Loc::getMessage('YANDEX_RUN_TITLE'), 0)?></div>
</div>
<?
