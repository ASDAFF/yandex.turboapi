...
$aTabs = array(
	...
	array("DIV" => "edit2", "TAB" => Loc::getMessage("YANDEX_TYRBO_API_TAB_FILTER"), "ICON"=>"main_user_edit", "TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_TAB_FILTER_TITLE")),
	array("DIV" => "edit3", "TAB" => Loc::getMessage("YANDEX_TYRBO_API_TAB_DEBUG"), "ICON"=>"main_user_edit", "TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_TAB_DEBUG_TITLE")),
	array("DIV" => "edit4", "TAB" => Loc::getMessage("YANDEX_TYRBO_API_TAB_PRODUCTION"), "ICON"=>"main_user_edit", "TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_TAB_PRODUCTION_TITLE")),
);
...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && check_bitrix_sessid())
{
	...
	{
		if ($apply != "")
		  LocalRedirect("/bitrix/admin/yandex_feed_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
		  LocalRedirect("/bitrix/admin/yandex_feed_list.php?lang=".LANG);
	}
	else
	{
...
if($IBLOCK_ID > 0) 
{
    $arTemplateList = \Yandex\TurboAPI\Turbo::getTemplateList();
	$ipropTemlates = new \Bitrix\Iblock\InheritedProperty\IblockTemplates($IBLOCK_ID);
...
$aMenu = array(
	array(
		...
		"LINK"=>"yandex_feed_list.php?lang=".LANG,
		"ICON"=>"btn_list",
	)
);
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"yandex_feed_edit.php?lang=".LANG,
		"ICON"=>"btn_new",
	);
	$aMenu[] = array(
		"TEXT"=>Loc::getMessage("YANDEX_TYRBO_API_DELETE"),
		"TITLE"=>Loc::getMessage("YANDEX_TYRBO_API_DELETE"),
		"LINK"=>"javascript:if(confirm('".Loc::getMessage("YANDEX_TYRBO_API_DELETE_CONF")."'))window.location='yandex_feed_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
...
<form id="feed_edit_form" method="POST" action="<?echo $APPLICATION->GetCurPage()?>" enctype="multipart/form-data" name="feed_edit_form">
	...
	if($IBLOCK_ID > 0)
	{
		...
		?>
        <tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_DESCRIPTION") ?></td>
            <td width="60%">
        ...
        <tr class="heading">
            <td colspan="2"><?=Loc::getMessage("YANDEX_TYRBO_API_FEED_1") ?></td>
        </tr>
        <tr>
            <th width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_CONTENT") ?></th>
            <td width="60%">
                <select name="CONTENT" id="CONTENT" style="width:45%">
                    <optgroup label="<?=Loc::getMessage("YANDEX_TYRBO_API_FILED_IBLOCK") ?>">
                        <option value="DETAIL_TEXT" <?= ($arFields['CONTENT'] == 'DETAIL_TEXT' ? 'selected' : ''); ?>>
                            <?=Loc::getMessage("YANDEX_TYRBO_API_DETAIL_TEXT") ?></option>
                        <option value="PREVIEW_TEXT" <?= ($arFields['CONTENT'] == 'PREVIEW_TEXT' ? 'selected' : ''); ?>>
                            <?=Loc::getMessage("YANDEX_TYRBO_API_PREVIEW_TEXT") ?></option>
                    </optgroup>
                    <?if(!empty($arProps)):?>
                        <optgroup label="<?=Loc::getMessage("YANDEX_TYRBO_API_PROPERTY_IBLOCK") ?>">
        ...
        <tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_PUB_DATE")?></td>
            <td width="60%">
		...
		<tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_PROPERTY_TITLE")?></td>
            <td width="60%">
		...
		<tr>
            <td width="40%"><?=Loc::getMessage("YANDEX_TYRBO_API_OFFERS_PROPERTY_TITLE")?></td>
            <td width="60%">
		...
		<tr>
			<td colspan="2" align="center" id="fieldset-item-add-button">
				<br>
				<button class="adm-btn" onclick="customFieldAdd(this); return false;"><?=Loc::getMessage('YANDEX_TYRBO_API_HEADING_FIELD_ADD')?></button>
			</td>
		</tr>
		<tr class="heading">
	...
	$tabControl->BeginNextTab();
	?>
	<input class="adm-btn-save" value="<?=Loc::getMessage("YANDEX_TYRBO_API_RUN")?>" onclick="generateTurboFeed('<?=$ID?>', 'debug_run')" name="debug_run" id="feed_debug_run_button_<?=$ID?>" type="button">
	<?
	$tabControl->BeginNextTab();
	?>
	<input class="adm-btn-save" value="<?=Loc::getMessage("YANDEX_TYRBO_API_RUN")?>" onclick="generateTurboFeed('<?=$ID?>', 'production_run')" name="production_run" id="feed_production_run_button_<?=$ID?>" type="button">
	<?
	$tabControl->Buttons(
	  array(
		"disabled"=>false,
		"back_url"=>"yandex_feed_list.php?lang=".LANG,
