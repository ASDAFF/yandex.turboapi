...
use Yandex\Export\TurboProfileTable;
...
{
	\CAdminMessage::ShowMessage(Loc::getMessage('YANDEX_ERROR_PROFILE_NOT_FOUND'));
	die();
}
...
if($_REQUEST['action'] == 'xml_export_run' && check_bitrix_sessid())
{
	...
	$profileExport = new \Yandex\Export\ProfileExport($arProfile['ID'], array('TMP_FILE_PATH' => $NS['TMP_FILE_PATH']));
	...
	{
		\CAdminMessage::ShowMessage(Loc::getMessage('YANDEX_ERROR_FILE_NOT_ELEMENTS'));
		die();
	}
	...
	if($v == $arValueSteps['init'])
	{
		if($NS['ELEMENTS_COUNT'] == 0)
		{
			...
			{
				\CAdminMessage::ShowMessage(Loc::getMessage('YANDEX_RUN_EXPORT_FLOCK'));
				die();
			}
			
			$NS['TIME_START'] = microtime(true);
			$NS['LOG_NAME'] = $logName = strftime('%Y-%m-%d_%H-%M-%S') . '__' . $profileExport->feed['ID'];
			\Yandex\Export\TurboProfileTable::update($profileExport->feed['ID'], array(
				'LAST_START' => new \Bitrix\Main\Type\DateTime(),
			));

			\Yandex\TurboAPI\Log::write(\Bitrix\Main\Localization\Loc::getMessage('YANDEX_TYRBO_API_LOG_EXPORT_PROFILE'), $profileExport->feed['ID'], $logName);
			...
			\Yandex\TurboAPI\Log::write(\Bitrix\Main\Localization\Loc::getMessage('YANDEX_TYRBO_API_LOG_LAST_START'), strftime('%d.%m.%Y %H:%M:%S'), $logName);
	...
	elseif($v < $arValueSteps['step_80'])
	{
		if($NS['ELEMENTS_COUNT'] < $profileExport->arResult['ALL_ELEMENTS_COUNT']) 
		{
			...
			\Yandex\TurboAPI\Log::write(
				\Bitrix\Main\Localization\Loc::getMessage('YANDEX_TYRBO_API_LOG_EXPORT_STEP', array('#STEP#' => $NS['STEP'], '#ITEMS#' => $arResult['LAST_ITEMS_COUNT'], '#MEMORY#' => \Yandex\TurboAPI\Log::getMemoryUsage())),
		...
		$msg = Loc::getMessage('YANDEX_RUN_RSS', array(
	...
	elseif($v < $arValueSteps['step_90'])
	{
		...
		$logName = $NS['LOG_NAME'];
		\Yandex\TurboAPI\Log::write(\Bitrix\Main\Localization\Loc::getMessage('YANDEX_TYRBO_API_LOG_LAST_END'), strftime('%d.%m.%Y %H:%M:%S'), $logName);
		\Yandex\TurboAPI\Log::write(\Bitrix\Main\Localization\Loc::getMessage('YANDEX_TYRBO_API_LOG_EXPORT_TOTAL_ELEMENTS'), $NS['ITEMS_COUNT'], $logName);
	...
	if($v == $arValueSteps['index'] || $profileExport->arErrors)
	{
		if($profileExport->arErrors && is_array($profileExport->arErrors))
		{
			if($msg)
			{
				echo Turbo::showProgress($msg, Loc::getMessage('YANDEX_RUN_TITLE_XML'), $v);
			}
	...
	if(isset($e))
	{
		echo Turbo::showProgress($msg, Loc::getMessage('YANDEX_RUN_TITLE_XML'), $e);
		if($e == $arValueSteps['index'])
		{
			unset($e);
		}
