...
if($_REQUEST['action'] == 'production_sections_run' && check_bitrix_sessid())
{
	global $arErrors;
	$arValueSteps = array(
		'init' => 0,
	...
	if($v == $arValueSteps['init'])
	{
		...
		if($arFeed['IS_NOT_UPLOAD_FEED'] == 'Y')
		{
			...
			$msg = Loc::getMessage('YANDEX_RUN_UPLOAD_FEED', array(
	...
	if($v == $arValueSteps['index'] || $arErrors)
	{
		if($arErrors && is_array($arErrors))
		{
			...
			{
				 \CAdminMessage::ShowMessage(Loc::getMessage('YANDEX_ERROR_UPLOAD_ADDRESS_P', array(
					"#FILE#" => $k,
	...
	if(isset($e))
	{
		echo Turbo::showProgress($msg, Loc::getMessage('YANDEX_RUN_TITLE'), $e);
		if($e == $arValueSteps['index'])
		{
			unset($e);
		}
	...
	if($v < $arValueSteps['index'])
	{
		?>
		<script>
		top.BX.runFeed(<?=$ID?>, 'production_sections_run', <?=$v?>, '<?=$PID?>', <?=CUtil::PhpToJsObject($NS)?>);
		</script>
		<?
	}
	else
